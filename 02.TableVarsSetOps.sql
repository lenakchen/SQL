...
So, it sorts the result, looks for adjacent pairs 
that are the same and eliminates all but
...
/* if we want to have the duplicates in our result, 
we add to union the word all 
that will turn the set operator into what's technically 
a multi-set operator that retains duplicates.
...
/* This query is going to get the IDs of all students 
who have applied to both CS for a major and EE for a major.
...
doesn't mean there's not another pair with the same student
where they applied to CS and EE.
All this is actually finding is students who applied to CS
and also applied to another major that's not EE.
